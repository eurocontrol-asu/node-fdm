...
Aircraft trajectory prediction is a fundamental research question in modern Air Traffic Management (ATM), enabling applications ranging from collision risk modelling, capacity prediction, or fuel efficiency monitoring. However, accurate flight dynamics modelling typically requires proprietary manufacturer performance data or complex kinetic models.

`node-fdm` is a Python library designed to bridge the gap between data-driven deep learning and classical aeronautical physics. It leverages Neural Ordinary Differential Equations (Neural ODEs) [@chen2018neural] to learn aircraft dynamics from observational data. By coupling physical laws (equations of motion) with neural networks, `node-fdm` allows researchers to reconstruct coherent flight profiles, infer missing aerodynamic parameters, and simulate counterfactual trajectories. The library supports end-to-end pipelines for both fully open Automatic Dependent Surveillance--Broadcast (ADS-B) data and proprietary Quick Access Recorder (QAR) streams.

# Statement of need

In the domain of ATM research, trajectory modelling can be addressed by two wide categories of methods. On one side, kinetic models like BADA (Base of Aircraft Data) [@nuic2010bada] or OpenAP [@sun2020openap] rely on explicit coefficients and assumptions that may not capture the idiosyncrasies of specific airframes or weather conditions. On the other side, pure deep learning approaches (with VAEs or LSTMs) often violate physical constraints, leading to unrealistic energy states or vertical profiles.

`node-fdm` addresses these limitations by providing a reproducible, open-source framework for **physics-guided** learning. It is designed for researchers who need:

- **Data Versatility:** The ability to ingest noisy, sparse public data (ADS-B) where key parameters like mass and thrust are missing, as well as high-fidelity private data (QAR);
- **Physical Consistency:** Unlike standard regression models, `node-fdm` solves the initial value problem using ODE solvers, ensuring that the generated trajectories are continuous and physically plausible;
- **Reproducibility:** A major hurdle in ATM research is the lack of standardized training pipelines. `node-fdm` provides complete workflows, from data download and smoothing to training and evaluation, allowing the community to benchmark new architectures against established baselines.
...
The package is implemented with PyTorch and is organized around four core abstractions designed for modularity and extensibility:

### Column Schema and Units
...
- Automated segment filtering to remove inconsistant data points;
