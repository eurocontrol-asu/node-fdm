...
def plot_speed_sources(df: pl.DataFrame, out_path: Path | None = None) -> None:
    ...
    bds_mach_clean, mach_outliers = _clean_bds(
        bds_mach, window=50, k=3.0,
        point_jump_max=0.05,
        zigzag_jump_min=0.05, zigzag_half_window=15, zigzag_density_min=0.25,
    )
    ...
    bds_ias_clean, ias_outliers = _clean_bds(
        bds_ias, window=50, k=3.0,
        point_jump_max=20.0,
        zigzag_jump_min=20.0, zigzag_half_window=15, zigzag_density_min=0.25,
    )
    ...
    bds_tas = df["bds_tas_kt"].to_numpy() if "bds_tas_kt" in df.columns else np.full(n, np.nan)
    era_tas = df["era_tas_kt"].to_numpy() if "era_tas_kt" in df.columns else np.full(n, np.nan)
    bds_tas_clean, tas_outliers = _clean_bds(
        bds_tas, window=50, k=3.0,
        point_jump_max=20.0,
        zigzag_jump_min=20.0, zigzag_half_window=15, zigzag_density_min=0.25,
    )
    era_tas_clean, era_tas_outliers = _clean_era(
        era_tas, window=15, k=3.0,
        zigzag_jump_min=20.0, zigzag_half_window=15, zigzag_density_min=0.15,
    )
    cleaned_filled_tas = _fill_bds_with_era(bds_tas_clean, era_tas_clean)
    cleaned_filled_tas = _hampel_filter(cleaned_filled_tas, window=5, k=3.0)
    cleaned_filled_tas = _interpolate_short_gaps(cleaned_filled_tas, max_gap=10)
    cleaned_filled_tas[on_ground] = np.nan

    # Derive TAS from cleaned Mach and cleaned CAS using ERA temperature/altitude.
    temp_k = df["era_temp_K"].cast(pl.Float64).to_numpy() if "era_temp_K" in df.columns else np.full(n, np.nan)
    alt_m = alt * FT_TO_M
    tas_from_mach_kt = mach_to_tas_real(cleaned_filled_mach, temp_k) * MS_TO_KT
    tas_from_cas_kt = cas_to_tas_real(cleaned_filled_ias * KT_TO_MS, alt_m, temp_k) * MS_TO_KT
    ...
    # --- TAS ---
    ax = axes[2]
    ax.plot(t, bds_tas, color="#e74c3c", linewidth=0.6, alpha=0.6, label="BDS TAS (raw)")
    ax.plot(t, era_tas, color="#3498db", linewidth=0.8, alpha=0.7, label="ERA TAS")
    ax.plot(t, cleaned_filled_tas, color="#27ae60", linewidth=1.2, label="BDS cleaned + filled w/ ERA")
    ax.plot(t, tas_from_mach_kt, color="#f39c12", linewidth=1.0, alpha=0.85,
            label="TAS from Mach_clean (ERA T)")
    ax.plot(t, tas_from_cas_kt, color="#16a085", linewidth=1.0, alpha=0.85, linestyle="--",
            label="TAS from CAS_clean (ERA T,h)")
    if tas_outliers.any():
        ax.scatter(t[tas_outliers], bds_tas[tas_outliers], color="#c0392b", s=12,
                   marker="x", label=f"BDS outliers ({tas_outliers.sum()})", zorder=5)
    if era_tas_outliers.any():
        ax.scatter(t[era_tas_outliers], era_tas[era_tas_outliers], color="#8e44ad", s=12,
                   marker="+", label=f"ERA outliers ({era_tas_outliers.sum()})", zorder=5)
    n_out = tas_outliers.sum()
    pct = 100 * n_out / max(1, (~np.isnan(bds_tas)).sum())
    ax.set_title(f"TAS  (BDS out: {n_out} / {pct:.1f}%, ERA out: {era_tas_outliers.sum()})", fontsize=10)
    ax.set_ylabel("Speed (kt)")
    ax.set_xlabel("Time (min)")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.2)
...
def main() -> None:
    ...
    for fid in flight_ids:
        ...
        out = Path(f"data/figures/speeds_v3_{fid}.png")
