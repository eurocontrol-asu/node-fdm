    ...
    def __init__(self, mean_dict: Dict[Any, Any], std_dict: Dict[Any, Any]):
        ...
        for k in mean_dict:
            ...
            self.register_buffer(
                f"mean_{k}", torch.tensor(mean_dict[k], dtype=torch.float32)
            )
            self.register_buffer(
                f"std_{k}", torch.tensor(std_dict[k], dtype=torch.float32)
            )
...
class OutputDenormalizer(nn.Module):
    ...
    def __init__(
        ...
        for k in mean_dict:
            ...
            self.register_buffer(
                f"max_{k}", torch.tensor(max_dict[k], dtype=torch.float32)
            )
