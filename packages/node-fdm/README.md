...
| `trainer` | `ODETrainer` + `TrainingConfig` — ODE rollout loss with per-variable `alpha_dict` weighting, optional tracking loss on autopilot targets (`lambda_tracking`), projected integration via `ClampedEuler`/`ClampedRK4` when `x_bounds` present, `grad_clip_norm` default 10.0, optional class-imbalance rebalancing via `use_mode_weights` (effective-number weights from `fdm_mode_label`), model weights + optimizer checkpoint save/load; hybrid architectures (`flight_feature_cols` non-empty) instantiate a `MassEncoderLinear`, optimize it jointly with the backbone, persist `mass_encoder.pt` next to layer checkpoints, log per-epoch `mass_encoder_coefs`, and expose `ODETrainer.identifiability_test(factor=1.3)` (§11.1 gate: returns `{baseline_mse, perturbed_mse, ratio}`) |
...
output columns listed in `DERIVED_FEATURES`, including `fdm_t_minus_d_norm`,
`fdm_lift_residual_norm` and `fdm_cl_residual`, can be requested through
`derived_cols`.
