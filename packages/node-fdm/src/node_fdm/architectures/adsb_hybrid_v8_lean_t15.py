"""NODE_ADSB_HYBRID_V8_LEAN_T15 — lean 2-feature MassEncoder, T=1.5.

Sibling of :mod:`node_fdm.architectures.adsb_hybrid_v7_lean`. Same
2-feature input set and Newton-mode physics, but the sigmoid temperature
drops from ``3.0`` to ``1.5`` so the encoder can output the full
``[OEW, MTOW]`` range when the linear drive justifies it.

Experiment 04 (T=3) reached corr 0.467 with predictions compressed in
``[58.6, 64.2]`` kg vs truth ``[52.6, 72.3]`` kg — i.e. under-confident.
The saturation-pile-up problem T=3 was designed for disappears with only
2 features, so a tighter temperature should expand the prediction range
and lift corr toward the 0.608 oracle ceiling.
...
NODE_ADSB_HYBRID_V8_LEAN_T15 = ArchitectureSpec(
    name="node_adsb_hybrid_v8_lean_t15",
    x_cols=X_COLS,
    u_cols=U_COLS,
    e0_cols=E0_COLS,
    e1_cols=E1_COLS,
...
register(NODE_ADSB_HYBRID_V8_LEAN_T15)
