"""NODE_ADSB_HYBRID_V10_PS_AUXLOSS — v9 + Poll-Schumann Eq 100 aux loss.

Sibling of :mod:`node_fdm.architectures.adsb_hybrid_v9_causal`. Same
3-feature causal MassEncoder (T=1.5, signs +/-/-) and same Newton
physics — the only difference is an additional auxiliary supervision
signal *during training* derived from the algebraic Poll-Schumann
optimum-cruise mass inversion (Eq 100, Part 1).

The aux loss is configured at the trainer level (``--lambda-aux``) and
applied as ::

    L_total = L_traj + λ_aux · MSE(m_pred, m_PS_eq100) / (MTOW - OEW)²

restricted to **cruise-stable** segments (alt ≥ 9000 m, |dh/dt| < 1 m/s)
where the optimum theory applies, and to FL_obs inside the invertible
range of Eq 100 for the A320 ψ-set. ``m_PS`` is computed at training
time *from observed altitude alone* — no QAR / ground-truth mass leak
(R5 preserved).

Auto-registers at import time. Phase 1.7 / Strategy E ticket.
"""
...
NODE_ADSB_HYBRID_V10_PS_AUXLOSS = ArchitectureSpec(
    name="node_adsb_hybrid_v10_ps_auxloss",
    x_cols=X_COLS,
    u_cols=U_COLS,
    e0_cols=E0_COLS,
    e1_cols=E1_COLS,
...
register(NODE_ADSB_HYBRID_V10_PS_AUXLOSS)
