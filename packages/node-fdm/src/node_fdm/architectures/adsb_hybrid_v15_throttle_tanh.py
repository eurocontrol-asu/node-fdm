"""NODE_ADSB_HYBRID_V15_THROTTLE_TANH — Phase 7 Strategy B : tanh-stretched throttle.
...
``0.1 + 0.9 * sigmoid(z)`` (v14) to a **stretched tanh** :

    χ = 0.55 + 0.45 · tanh(z / 2.5)

with ``dχ/dz = (0.45 / 2.5) · sech²(z / 2.5)`` ≈ 0.18 at the centre and
≈ 0.03 at the extrema — **x6 less compressed** than the sigmoid's x13
attenuation. Falls back from Strategy A (linear clamp,
:mod:`adsb_hybrid_v15_throttle_clamp`) when the clamp's hard gradient
discontinuity at z = ±5 is suspected of destabilising training without
unlocking the χ extrema.
...
def _v15_layers_with_tanh() -> list[LayerSpec]:
    """Build the v15-tanh layer list : v14 layers + physics ``throttle_activation``."""
    layers: list[LayerSpec] = []
    for layer in NODE_ADSB_HYBRID_V14_PSEFFICIENCY.layers:
        if layer.name == "physics":
            new_config = dict(layer.config)
            new_config["throttle_activation"] = "tanh_stretched"
            layers.append(
                LayerSpec(
                    name=layer.name,
...
NODE_ADSB_HYBRID_V15_THROTTLE_TANH = ArchitectureSpec(
    name="node_adsb_hybrid_v15_throttle_tanh",
...
register(NODE_ADSB_HYBRID_V15_THROTTLE_TANH)
