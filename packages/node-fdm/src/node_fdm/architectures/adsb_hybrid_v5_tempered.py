"""NODE_ADSB_HYBRID_V5_TEMPERED — 9-feature MassEncoder with T=3 sigmoid.

Sibling of :mod:`node_fdm.architectures.adsb_hybrid_v4`. Same feature set
(9 features, Newton-mode physics) but the MassEncoder uses
:class:`MassEncoderLinearTempered` with ``temperature = 3.0`` to broaden
the sigmoid response and diagnose / suppress the bimodal OEW/MTOW
saturation pathology flagged in Experiment 01.

Identifiability invariant (AC4): the longitudinal NN backbone still sees
only the baseline inputs — no mass, no flight features. The only knob
changed vs v4 is the encoder's sigmoid temperature.
...
NODE_ADSB_HYBRID_V5_TEMPERED = ArchitectureSpec(
    name="node_adsb_hybrid_v5_tempered_t3",
    x_cols=X_COLS,
    u_cols=U_COLS,
    e0_cols=E0_COLS,
    e1_cols=E1_COLS,
...
register(NODE_ADSB_HYBRID_V5_TEMPERED)
