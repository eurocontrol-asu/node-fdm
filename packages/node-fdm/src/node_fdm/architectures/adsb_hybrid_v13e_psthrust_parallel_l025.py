"""NODE_ADSB_HYBRID_V13E_PSTHRUST_PARALLEL_L025 — Strategy W with λ=0.25.

Variant of v13d with the static parallel-head weight reduced from
0.5 → 0.25. Tests whether a smaller unbounded escape preserves
AC5 (NN-off val_loss doesn't collapse) while still closing R7.

Per Exp 19, v13d's parallel head at λ=0.5 caused +330 % AC5
degradation — the closed-loop heavily co-adapted. λ=0.25 should
reduce the parallel head contribution by half, shifting more work
back to the analytical T_PS prior.

Trigger column : `fdm_t_correction_parallel_l025` (PhysicsLayer
dispatches to the v13e branch with λ=0.25 when present).
"""
...
NODE_ADSB_HYBRID_V13E_PSTHRUST_PARALLEL_L025 = ArchitectureSpec(
    name="node_adsb_hybrid_v13e_psthrust_parallel_l025",
    x_cols=X_COLS,
    u_cols=U_COLS,
    e0_cols=E0_COLS,
    e1_cols=E1_COLS,
        ...
        LayerSpec(
            ...
            config={
                "denormalize_modes": {
                    ...
                    "fdm_t_correction_parallel_l025": "scaled",
                    "fdm_t_minus_d_norm_parallel_l025": "scaled",
                },
                "scale_overrides": {
...
register(NODE_ADSB_HYBRID_V13E_PSTHRUST_PARALLEL_L025)
